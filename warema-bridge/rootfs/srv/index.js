        ...
        switch (msg.topic) {
            ...
            case 'wms-vb-blind-position-update':
                ...
                if (typeof msg.payload.angle !== "undefined") {
                    ...
                    let new_tilt = 100 - parseInt(msg.payload.angle);
                    client.publish('warema/' + msg.payload.snr + '/tilt', '' + new_tilt, {retain: true})
                }
                break;
            default:
...
client.on('message', function (topic, message) {
    ...
    switch (command) {
        ...
        case 'set_position':
            ...
            break;
        case 'set_tilt':
            let set_tilt = 100 - parseInt(message);
            log.info('Setting ' + device + ' to ' + set_tilt + '°, position ' + devices[device].position);
            stickUsb.vnBlindSetPosition(device, parseInt(devices[device]['position']), set_tilt);
            break;
        default:
